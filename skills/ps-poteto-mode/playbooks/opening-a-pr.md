...
**PRs.** Run `/deslop` from `cursor-team-kit` over the diff before commit. Run `/ps-no-comments` before review. Write every PR title, PR description, and commit body with `/ps-technical-writing`, then apply `/ps-unslop`. Apply every technical-writing layer except Diátaxis. Use one word for each action, keep articles, and avoid `-ing` when a plain verb works.

**Titles.** Use Conventional Commits in the form `type(scope): subject`. Use `feat`, `fix`, `docs`, `refactor`, `test`, `chore`, or `perf` as the type. Use the changed area, such as `pstack` or `ps-poteto-mode`, as the scope. Keep the subject short and imperative. Name a real symbol when one carries the change. For example, `fix(pstack): retarget opening-a-pr babysit trigger`. Do not add a trailing period.
...
A subagent that opens a PR runs `ps-interrogate`, `/deslop`, and `/ps-no-comments`. It returns the URL and does not babysit. Return to the parent.
