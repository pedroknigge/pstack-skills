...
3. Plan the fix from the trace. If it crosses a function boundary, `ps-architect` first. Delegate implementation to a subagent using your configured perf-issue model (default `grok-4.6-fast-xhigh`). Review the diff. Capture a post-fix trace.
   Apply the **ps-principle-sequence-verifiable-units** skill, verifying each attempt before trying the next.
