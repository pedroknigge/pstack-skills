...
- Shared state: if two actors might both write, ask "what happens?" If the answer isn't "nothing," default to per-actor state with a merge at the read boundary, per the **ps-principle-separate-before-serializing-shared-state** skill.
...
- Encode invariants in types: hard-to-misuse types > runtime checks > prose comments, per the **ps-principle-encode-lessons-in-structure** skill.
- Validate at boundaries, trust types inside, per the **ps-principle-boundary-discipline** skill. Business logic as pure functions. The shell stays thin.
- Single source of truth per invariant. Derive instead of sync.
- Idempotent state transitions where applicable, per the **ps-principle-make-operations-idempotent** skill. Ask what happens if the operation runs twice or crashes halfway.
- Short call chains. If tracing the flow needs more than three files, flatten the hierarchy, per the **laziness-protocol** and **ps-principle-minimize-reader-load** skills.
