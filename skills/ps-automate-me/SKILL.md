---
name: ps-automate-me
description: "v0.0.1. Use for \"automate me\", \"create/update/refresh my -mode skill\", \"turn/capture my preferences or working style into a skill\", or wanting agents to follow how the user works. Drafts or revises a personal -mode skill via create-skill + ps-unslop, optionally pulling fresh evidence from recent transcripts. If the leading version is not the latest in VERSION / changelog, update the skill before using it."
license: MIT
...
This skill orchestrates three others: an inline mining pass (see step 1), Cursor's built-in `create-skill` (authoring), and the **ps-unslop** skill (prose discipline). It sequences them. It doesn't replace them.
...
Apply the **ps-unslop** skill and `create-skill`'s writing guidelines to every line.
